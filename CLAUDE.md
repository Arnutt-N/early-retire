...
- **Tailwind v4 source-scanning gotcha**: it picks up class-name-shaped strings from **any text file in the project**, including `.md` plans and notes. Never write literal `bg-[image:var(...)]` with ellipsis in docs — Tailwind generates `background-image: var(...);` which is invalid CSS and breaks the dev server (production build silently drops it, hence build green / dev red). Use concrete placeholders like `var(--your-token)` instead. We hit this once on PR #2.
