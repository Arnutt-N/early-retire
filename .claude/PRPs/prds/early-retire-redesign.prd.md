...
|---|-------|-------------|--------|----------|---------|----------|
| 1 | Foundation & Design Tokens | `globals.css` (gradient/motion/elevation tokens), `types/index.ts` (mode/overrides/schemaVersion), `lib/calculations.ts` (leave deduction + overrides signatures) | complete | - | - | `.claude/PRPs/plans/phase-1-foundation.plan.md` (PR #3) |
| 2A | DatePickerTH Rewrite | New `components/ui/CalendarPickerTH.tsx` — calendar overlay พ.ศ. + keyboard fallback + reduced-motion aware | complete | with 2B | 1 | `.claude/PRPs/plans/phase-2a-datepicker.plan.md` (PR #4) |
| 2B | UI Primitives Update | Button/Card/Input/Tooltip refresh + new `SegmentedControl` + `ProgressBar` deferred to 3+4 | complete | with 2A | 1 | `.claude/PRPs/plans/phase-2b-ui-primitives.plan.md` (PR #5) |
| 3+4 | Step Components + Page Wiring (combined) | new `ModeSelect.tsx`, all sections rewritten, `app/page.tsx` 6-step flow + `__schemaVersion` migration + useMemo rebuild + `ProgressBar` 6 labels + e2e rewritten | complete | - | 2A, 2B | (combined; no separate plan file — implemented from PRD spec + JIT design) (PR #6) |
| 5 | Print Stylesheet + AnimatedNumber | `@media print` A4 export rules + new `AnimatedNumber` component for hero stat counters; tooltip help text deferred (Tooltip primitive ready but not wired) | complete | with 6 | 4 | (no separate plan file) (PR #7) |
| 6 | Wrap-up | CLAUDE.md updated for redesign-era architecture, PRD marked shipped, schema-migration E2E added | complete | with 5 | 4 | (this PR) |
...
*Status: SHIPPED 2026-04-26 — all 7 phase PRs merged to main; awaiting owner self-test for sign-off*
